...
- Replacement writes a sibling temporary file, flushes it with `fsync`, closes it, rechecks the parent/target identity, and performs one same-directory rename. On Windows, Node/libuv maps this to `MoveFileExW(..., MOVEFILE_REPLACE_EXISTING)`; the implementation never moves the live file aside. Transient `EPERM`/`EACCES`/`EBUSY` errors are retried within a bounded limit. A failed replacement preserves the temporary recovery path in the error.
...
- Запись выполняется через временный файл рядом с целью: `fsync`, закрытие, повторная проверка родителя и цели, затем одно переименование. В Windows Node/libuv использует `MoveFileExW(..., MOVEFILE_REPLACE_EXISTING)`; живой файл никогда не переносится в сторону. Временные `EPERM`/`EACCES`/`EBUSY` повторяются ограниченное число раз, а при ошибке путь к временному файлу сохраняется в сообщении.
