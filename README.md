...
- Restore verifies the snapshot hash, refuses an invalid target, and refuses to proceed when the Windows process check cannot establish that PEPPERED is closed. The process check parses complete `tasklist /FO CSV` rows and compares only the exact game image names, so `PEPPERED Save Manager.exe` does not block its own restore.
- Before replacement, a different current save is written as a fresh automatic **Recovery copy** beside any existing automatic recoveries, even when the same bytes also exist as a manual snapshot. Active replacement starts only after that candidate is valid; if replacement fails, the candidate is atomically quarantined before cleanup, a rename failure leaves the valid candidate visible, partial quarantine cleanup reports only residue, and prior recoveries remain untouched. After a successful replacement, old automatic recoveries are removed sequentially, so a clean success leaves exactly one visible recovery; a cleanup failure is reported as partial success with the remaining valid copies available for convergence on a later differing restore. Recovery labels are localized in the UI; renaming one makes it manual and removes it from the rolling slot.
...
- Перед восстановлением проверяется хеш точки и корректность цели. На Windows действие закрывается, если нельзя подтвердить, что PEPPERED завершён. Проверка разбирает полные строки `tasklist /FO CSV` и сравнивает только точные имена процессов игры, поэтому `PEPPERED Save Manager.exe` больше не блокирует собственное восстановление.
- Отличающееся текущее сохранение сначала записывается как новая автоматическая **копия для восстановления** рядом с уже существующими автоматическими копиями, даже если такие байты уже есть в обычной точке. Замена активного файла начинается только после проверки кандидата; при ошибке кандидат сначала атомарно перемещается в карантин: при сбое переименования валидный кандидат остаётся видимым, частичная очистка карантина сообщает только об остатках, а прежние копии не затрагиваются. После успешной замены старые автоматические копии удаляются последовательно: чистый успех оставляет ровно одну видимую копию, а ошибка очистки явно показывается как частичный успех и оставляет валидные копии для объединения при следующем отличающемся восстановлении. Название копии локализуется в интерфейсе; переименование делает её обычной точкой и освобождает автоматическое место.
